...
#Save images
def save_images(save_img_path, epoch, real_A, fake_B, real_B, fake_A, summary):
    n, c, h, w = real_A.size()
    samples = torch.FloatTensor(4*n, c, h, w).zero_()
    for i in range(n):
        samples[4*i + 0] = real_A[i].data
        samples[4*i + 1] = fake_B[i].data
        samples[4*i + 2] = real_B[i].data
        samples[4*i + 3] = fake_A[i].data


    images = utils.make_grid(samples, nrow=8, padding=30, normalize=True)
    summary.add_image("samples", images, epoch)
    file_name = save_img_path
